...
test.describe('account panel', () => {
  // Test case: XCM006
  test('should transfer ASTR tokens from Alice to Acala', async ({ page, context }) => {
    ...
    await page.getByPlaceholder('Destination Address').fill(ALICE_ADDRESS);
    await page.getByPlaceholder('0.0').fill(transferAmount.toString());
    await page.getByRole('button', { name: 'Confirm' }).click();

    const bobBalanceBeforeTransaction = await getBalance(ALICE_ADDRESS);
    await signTransaction(context);
    await page.waitForSelector('.four', { state: 'hidden' });

    await expect(page.getByText('Success')).toBeVisible();
    const bobBalanceAfterTransaction = await getBalance(ALICE_ADDRESS);
