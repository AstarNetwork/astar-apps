...
const TURING_PARA_ID = 2000;
const ASTAR_PARA_ID = 1000;

function getProxyAccount(api: any, address: string) {
...
export function useAutoCompound() {
  ...
  const compoundAll = async (): Promise<void> => {
    ...
    const oakAPI = await getAPI(oakEndpoint);
    ...
    if (isEmpty(proxiesResponse)) {
      const proxyAddress = getProxyAccount(api, address);
    ...
    const xcmpCall = oakAPI.tx.automationTime.scheduleXcmpTask(
      providedId,
      { Fixed: { executionTimes: [Math.floor(Date.now() / 1000) + 24 * 60 * 60] } }, // TODO: calculate execution time
      ASTAR_PARA_ID,
      0,
      encodedProxyCall,
    ...
    const turingAddress = encodeAddress(address, 51);
