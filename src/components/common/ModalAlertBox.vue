      ...
      >
        <div>
          <div>
            ...
            >{{ $t('balance.modals.alert') }}</h3>
            <div
              class="tw-mt-1 tw-w-full tw-rounded-md tw-bg-white dark:tw-bg-darkGray-900 dark:tw-text-white"
            >{{ msg }}</div>
          </div>
        </div>
        <div class="tw-mt-6 tw-flex tw-justify-center tw-flex-row-reverse" v-if="hasClose">
          <button type="button" @click="closeModal" class="close-modal">{{ $t('close') }}</button>
        </div>
      </div>
    </div>
  </div>
