  ...
  <div class="tw-bg-white dark:tw-bg-darkGray-800 tw-shadow tw-rounded-lg tw-p-5 tw-mb-5">
    ...
    <div class="tw-grid tw-grid-cols-1 tw-gap-2 tw-my-4">
      <div>
        ...
        >{{ $t('dapps.codeHash') }}</div>
        <div class="tw-flex tw-justify-between tw-items-center">
          <div class="tw-text-xs tw-text-blue-900 dark:tw-text-darkGray-100">{{ shortenCodeHash }}</div>
          ...
          <input type="hidden" id="hiddenCodeHash" :value="realCodeHash" />
        </div>
      </div>
    ...
    <div class="tw-text-right">
      <button type="button" class="coppy-abi-button" @click="copyABI">{{ $t('dapps.copyAbi') }}</button>
      <input type="hidden" id="hiddenAbi" :value="abi" />
      <button type="button" class="forget-button" @click="onForget">{{ $t('forget') }}</button>
    </div>
  </div>
</template>
<script lang="ts">
